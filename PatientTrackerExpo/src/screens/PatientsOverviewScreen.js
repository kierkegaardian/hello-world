...
export default function PatientsOverviewScreen({ navigation }) {
  ...
  useEffect(() => {
    if (!hospitalId) { setFloors([]); setFloorId(null); return; }
    db.transaction(tx => {
  ...
  const loadPatients = useCallback(() => {
    db.transaction(tx => {
      ...
      if (floorId) { where.push('p.floor_id = ?'); params.push(floorId); }
      if (filter) { where.push('LOWER(p.name) LIKE ?'); params.push(`%${filter.trim().toLowerCase()}%`); }
      const sql = 'SELECT p.id, p.name, p.retired, p.medical_issue, p.hospital_id, h.name AS hospital_name, f.floor_number ' +
        'FROM patients p LEFT JOIN hospitals h ON h.id = p.hospital_id LEFT JOIN floors f ON f.id = p.floor_id ' +
        (where.length ? 'WHERE ' + where.join(' AND ') + ' ' : '') + 'ORDER BY h.name ASC, p.name ASC;';
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerRight: () => (
        <Button title="Add" onPress={() => navigation.navigate('AddPatient', {
  ...
  const renderItem = ({ item }) => (
    <View style={{ paddingVertical: 6, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
      <TouchableOpacity style={{ flex: 1 }} onPress={() => navigation.navigate('AddPatient', { patientId: item.id, hospital: item.hospital_id ? { id: item.hospital_id, name: item.hospital_name } : null, reload: loadPatients })}>
    ...
    <View style={{ flex: 1, padding: 16 }}>
      <View style={{ marginBottom: 8 }}>
        ...
        <Picker selectedValue={hospitalId} onValueChange={v => setHospitalId(v)}>
          ...
          {hospitals.map(h => (<Picker.Item key={h.id} label={h.name} value={h.id} />))}
        </Picker>
      </View>
      <View style={{ marginBottom: 8 }}>
        <Text>Floor</Text>
        <Picker selectedValue={floorId} onValueChange={v => setFloorId(v)} enabled={!!hospitalId}>
          ...
          {floors.map(f => (<Picker.Item key={f.id} label={`Floor ${f.floor_number}`} value={f.id} />))}
        </Picker>
      </View>
      ...
      <FlatList data={patients} keyExtractor={item => item.id.toString()} renderItem={renderItem} />
    </View>
  );
}
