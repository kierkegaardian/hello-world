...
export default function AddHospitalScreen({ navigation }) {
  ...
  const addHospital = () => {
    ...
    db.transaction(tx => {
      tx.executeSql('INSERT INTO hospitals (name, location) VALUES (?, ?);', [name, location], (_, result) => {
        const hospitalId = result.insertId;
        const floorCount = parseInt(numFloors, 10) || 0;
        for (let i = 1; i <= floorCount; i++) {
          const desc = floorDescriptions[i] || '';
          tx.executeSql('INSERT INTO floors (hospital_id, floor_number, description) VALUES (?, ?, ?);', [hospitalId, i, desc]);
        }
      });
  ...
  return (
    <ScrollView style={{ flex: 1, padding: 16 }}>
      ...
      <TextInput placeholder="Number of floors" value={numFloors} onChangeText={setNumFloors} keyboardType="numeric" />
