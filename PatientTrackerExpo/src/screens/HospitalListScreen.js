...
export default function HospitalListScreen({ navigation }) {
  ...
  const loadHospitals = useCallback(() => {
    db.transaction(tx => {
      ...
      const base = 'SELECT h.id, h.name, h.location, h.retired, ' +
        '(SELECT COUNT(*) FROM patients p WHERE p.hospital_id = h.id) AS patientCount ' +
        'FROM hospitals h ';
      const where = [];
      const params = [];
      if (!showRetired) where.push('h.retired = 0');
      if (filter) {
        const q = `%${filter.trim().toLowerCase()}%`;
        where.push('(LOWER(h.name) LIKE ? OR LOWER(h.location) LIKE ?)');
        params.push(q, q);
      }
      const sql = base + (where.length ? 'WHERE ' + where.join(' AND ') + ' ' : '') + 'ORDER BY h.name ASC;';
  ...
  useEffect(() => { loadHospitals(); }, [filter, showRetired, loadHospitals]);
  useFocusEffect(useCallback(() => { loadHospitals(); }, [loadHospitals]));
          ...
          <Button
            ...
            onPress={() => {
              Alert.alert('Reset Data', 'This will clear and reseed local data.', [
                ...
                {
                  text: 'Reset', style: 'destructive', onPress: async () => { await resetDatabase(); loadHospitals(); },
                },
              ]);
            }}
          />
  ...
  const toggleRetired = (hospital) => {
    db.transaction(tx => {
      tx.executeSql('UPDATE hospitals SET retired = ? WHERE id = ?;', [hospital.retired ? 0 : 1, hospital.id], loadHospitals);
    });
  };
        ...
        renderItem={({ item }) => (
          <View style={{ paddingVertical: 6, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            <TouchableOpacity onPress={() => navigation.navigate('Patients', { hospital: item })} style={{ flex: 1 }}>
              <Text style={{ fontSize: 18, color: item.retired ? 'gray' : 'black' }}>{item.name} ({item.patientCount})</Text>
            </TouchableOpacity>
